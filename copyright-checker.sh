...
do
  ...
  do
    ...
    then
      ...
      #Notice,
      if grep -q "This repository is empty" $FILE_NAME
      then
        echo "Repository $LICENSE_URL is empty." >> $LOG_FILE
        rm $FILE_NAME
        continue
      fi
	...
	sed '/copyright owner\|copyright notice\|copyright license\|copyright statement\|distribute, all copyright\|copyright holder/Id' $FILE_NAME_TEMP > $FILE_NAME
	cp $FILE_NAME $FILE_NAME_TEMP
  ...
  then
    ...
    echo "$COMPONENT,$MESSAGE_LICENSE_FILE_NOT_FOUND,$LICENSE,No file is found containing keyword copyright(insensitively) in this repository" >> $CSV_FILE
    NUM_FAIL=$(($NUM_FAIL + 1))
  fi
done;
